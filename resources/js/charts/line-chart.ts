...
type ScaleOf<T extends Date | number> = T extends Date
...
export function makeOptionsDate(
  options: Partial<Options<Date>>,
): Options<Date> {
  return {
...
export function makeOptionsNumber(
...
function makeSharedDefaultOptions<X extends Date | number>(
  options: Partial<Options<X>>,
): Omit<Options<X>, 'scaleX'> {
  return {
    ...
    infoBoxFormatX:
      options.infoBoxFormatX ?? options.formatX ?? defaultFormatter,
    infoBoxFormatY:
      options.infoBoxFormatY ?? options.formatY ?? defaultFormatter,
...
export default class LineChart<X extends Date | number> {
  ...
  private readonly hoverArea: Selection<
    HTMLDivElement,
    unknown,
    null,
    undefined
  >;
  private readonly hoverCircle: Selection<
    HTMLDivElement,
    unknown,
    null,
    undefined
  >;
  private readonly hoverInfoBox: Selection<
    HTMLDivElement,
    unknown,
    null,
    undefined
  >;
  private readonly hoverInfoBoxX: Selection<
    HTMLDivElement,
    unknown,
    null,
    undefined
  >;
  private readonly hoverInfoBoxY: Selection<
    HTMLDivElement,
    unknown,
    null,
    undefined
  >;
  private readonly hoverLine?: Selection<
    HTMLDivElement,
    unknown,
    null,
    undefined
  ...
  constructor(
    ...
    this.area = select(area).classed(
      classWithModifiers(bn, this.options.modifiers),
      true,
    );

    this.svg = this.area.append('svg');

    this.svgWrapper = this.svg.append('g').classed(`${bn}__wrapper`, true);

    if (this.options.axisLabels) {
      this.axes = {
        svgX: this.svgWrapper
          .append('g')
          .classed(`${bn}__axis ${bn}__axis--x`, true),

        svgY: this.svgWrapper
          .append('g')
          .classed(`${bn}__axis ${bn}__axis--y`, true),

        x: axisBottom(this.options.scaleX).tickSizeOuter(0).tickPadding(5),

        y: axisLeft(this.options.scaleY).ticks(4),
      };
    }

    this.svgLine = this.svgWrapper.append('path').classed(`${bn}__line`, true);

    this.hoverArea = this.area
      .append('div')
    ...
    this.hover = this.hoverArea
      .append('div')
      .classed(`${bn}__hover`, true)
      .attr('data-visibility', 'hidden');

    if (this.options.circleLine) {
      this.hoverLine = this.hover
        .append('div')
        .classed(`${bn}__hover-line`, true);
    }

    this.hoverCircle = this.hover
      .append('div')
      .classed(`${bn}__hover-circle`, true);

    this.hoverInfoBox = this.hover
      .append('div')
      .classed(`${bn}__hover-info-box`, true)
      .attr('data-float', 'left');

    this.hoverInfoBoxX = this.hoverInfoBox
      .append('div')
      .classed(
        `${bn}__hover-info-box-text ${bn}__hover-info-box-text--x`,
        true,
      );

    this.hoverInfoBoxY = this.hoverInfoBox
      .append('div')
      .classed(
        `${bn}__hover-info-box-text ${bn}__hover-info-box-text--y`,
        true,
      );
  ...
  private drawAxes() {
    ...
    this.axes.svgY.transition().call(this.axes.y);
    ...
    this.axes.svgX.selectAll('.domain').classed('u-hidden', true);

    this.axes.svgY.selectAll('.domain').classed('u-hidden', true);
    ...
    this.axes.svgY.selectAll('text').classed(`${bn}__tick-text`, true);
  }

  private drawLine() {
    this.svgLine.transition().attr('d', this.line);
  }
  ...
  private readonly onHover = (event: DragEvent | MouseEvent) => {
    ...
    const d =
      x.valueOf() - this.data[i - 1].x.valueOf() <=
      this.data[i].x.valueOf() - x.valueOf()
        ? this.data[i - 1]
        : this.data[i];
  ...
  private setDimensions() {
    ...
    this.width =
      areaDims.width - (this.options.marginLeft + this.options.marginRight);
    this.height =
      areaDims.height - (this.options.marginTop + this.options.marginBottom);
