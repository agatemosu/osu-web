...
export default class BbcodeAutoPreview {
  ...
  private readonly loadPreview = (target: HTMLTextAreaElement) => {
    ...
    const preview = htmlElementOrNull(
      form.querySelector(".js-post-preview--preview"),
    );
    ...
    const xhr = $.post(route("bbcode-preview"), {
      text: body,
  ...
  private readonly onInput = (e: InputEvent) => {
    const target = htmlElementOrNull(e.target)?.closest(
